package com.qichong.util.redisutil;
...
public class JedisAdapter implements InitializingBean {
    ...
    /**
     *
     * @param name set 结合的名字
     * @param numbers 从set集合随机抽取的个数
     * @return 返回numbers个用户
     */
    public List<String> srandmember(String name,Integer numbers){
        Jedis jedis = null;
        try{
            jedis = pool.getResource();
            return jedis.srandmember(name,numbers);
        }catch (Exception e){
            logger.error("发生异常" + e.getMessage());
            return null;
        }finally {

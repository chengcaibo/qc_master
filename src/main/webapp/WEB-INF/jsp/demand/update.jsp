...
<body>
	...
	<form action="/updateDemandInfo" method="post"
		enctype="multipart/form-data">
		<div class="frame">
			<h1>修改我的需求</h1>
			<div class="frame-bottom">
				<input type="hidden" value="${demandInfo.id}" name="id" />
				<div class="textarea-box">
					<textarea id="content" maxlength="150"
						placeholder="请输入您的需求,例如：我需要一个懂算法，且懂架构的工程师。俩月" name="content">${demandInfo.content}</textarea>
				...
				<div id="file-headPortrait-parent">
					<label for="file-headPortrait"> <i></i> <img
						src="${demandInfo.picture}" title="点击上传图片"
				...
				<div class="reward">
					<input type="text" placeholder="请输入酬劳(元)" id="reward" name="reward"
						class="salary" value="${demandInfo.reward}"> <span>——</span>
					<input type="text" placeholder="请输入结束酬劳(元)" id="endReward"
						name="endReward" class="salary" value="${demandInfo.endReward}">
					<span class="choose-ok"> <input id="isMeeting"
						type="checkbox" name="meeting" class="checkbox"> <label
						for="isMeeting">可面议</label>
					</span>
						...
						<div class="form-group">
							<div style="position: relative; border: 1px solid #ccc;">
								<input id="city-picker3" class="form-control" readonly
									...
									data-toggle="city-picker" value="${demandInfo.address}">
							</div>
						</div>
					</div>
				...
				<div class="phone">
					<input type="text"
						...
						value="${demandInfo.detailedAddress}">
					<p class="detailedAddress-p">详细地址填写错误</p>
				</div>

				<button id="reset" type="reset">重置</button>
				<button type="submit" name="releaseRequirementsButton">修改</button>
			</div>
		</div>
	</form>

/**
 * real-name-index.js 用于主页的实名认证js
 */
// 查询对象
let query = {
	/** 获取地址 */
	...
	commonQuery (method, this_) {
		...
		$.ajax({
			url : `/admin/query_certification_${method}`,
			data : {
			...
			success (res) {
				...
				$(`#cert-${method}-count`).text(res.total);
				// 遍历
				$(`#cert-${method}-tbody`).empty();
				var list = res.list;
				if (list.length != 0) {
					for (let i in list)
						$(`#cert-${method}-tbody`).append(this_.template.one(list[i]));
				} else {
					$(`#cert-${method}-tbody`).append($(`#cert-${method}-tbody`).append(this_.template.two()));
				}
			},
			complete () {
				layer.close(loading)
			...
			one (info) {
				return `
							<tr>
								...
								<td>${info.realName}</td>
								<td>${info.identity}</td>
								<td class="cert-img"><img alt="身份证手持照片" src="${query.getUrl(info.identityPictureC.url)}"></td>
								<td>${new Date(info.createTime).format("yyyy年MM月dd日 hh:mm:ss")}</td>
								<td><a href="/admin/audits/realname/${info.id}" class="layui-btn layui-btn-normal">进入审核</a></td>
							</tr>`;
			},
			// 没有数据的
			two () {

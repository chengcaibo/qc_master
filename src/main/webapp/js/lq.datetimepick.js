...
$("#datetimepicker3").click(function(e) {
	e.stopPropagation();
	$(".close-calendar").css("display", "block");
	$(this).lqdatetimepicker({
		css : 'datetime-day',
		dateType : 'D',
		noPulsOne : true,
		selectback : datetimeCallBack
	});
});
...
$.fn.lqdatetimepicker.setDateData = function($this, _obj, _item, _this) {
	...
	if (typeof (_datetime) == 'object') {
		...
		for (var i = 0; i < _datetime.length; i++) {
			_time = $("<dd data-value=" + _datetime[i] + "><em>" + _datetime[i] + "<span class=\"make-span\"></span></em></dd>");
			if (_this.dateType == 'D') {
			...
			_time.on("click", function() {
				...
				// 自定义，渲染
				if (typeof printCanUseTime == "function") {
					setTimeout("printCanUseTime()", 50);
				}
			});
		...
		// 自定义，渲染
		if (typeof printCanUseDate == "function") {
			setTimeout("printCanUseDate()", 50);
		}
	}
}

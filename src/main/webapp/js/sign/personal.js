(function() {
		...
		personal : {
			...
			check : function() {
				...
				if (typeof ignoreVCode == "undefined")
					count += p.vCode.flag;
				count += p.luoValidation.flag;
				count += p.protocol.flag;
				var flag = (count == 0);
			...
			password : {
				obj : $(".sign-personal input[name=password]"),
				flag : 1,
				validation : function() {
					var p = io.personal.password;
					p.flag = 1;
					// 密码长度为6-18位，可以使用（！@#￥%&*-+=._）特殊符号
					var reg = /^[\w\d!@#\$%\^&\*\(\)\[\]\-\+=\.\,_~\?]{6,18}$/;
					if (p.obj.val() == "") {
						io.showDialog(p.obj.parent().parent(), "请输入您的密码", "error");
					} else if (!reg.test(p.obj.val())) {
						io.showDialog(p.obj.parent().parent(), "密码格式有误，长度为6-18位，可以使用!@#$%^&*()[]-+=.,_~特殊符号", "error");
					} else {
						// 验证通过
						p.flag = 0;
						io.personal.check();
						io.showDialog(p.obj.parent().parent(), "密码可以使用，请牢记", "success");
						return true;
					}
					return false;
				}
			...
			email : {
				obj : $(".sign-personal input[name=email]"),
				flag : 1,
				validation : function() {
			...
			luoValidation : {
				obj : $(".sign-personal input[name=luotest_response]"),
				getDialog : function() {
					return io.personal.luoValidation.obj.parent().parent().parent();
				},
				flag : 1,
				ajax : function() {
					if (!luoFlag) return;
					var p = io.personal.luoValidation;
					p.flag = 1;
					$.ajax({
						url : "/api/validation",
						dataType : "JSON",
						...
						success : function(r) {
							if (r.ret == qc.retEnum.SUCCESS) {
								p.flag = 0;
								io.personal.check();
								io.showDialog(p.getDialog(), "验证通过", "success");
							} else {
								io.showDialog(p.getDialog(), "验证失败，请重新完成人机验证", "error");
								LUOCAPTCHA.reset();
								luoFlag = false;
							}
						},
			...
			protocol : {
				obj : $(".sign-personal input[name=protocol]"),
				flag : 0,
				validation : function() {
					var p = io.personal.protocol;
					p.flag = p.obj.get(0).checked ? 0 : 1;
					io.personal.check();
				}
			}
		},
	...
	// 绑定[个人-密码]事件
	temp = io.personal.password;
	temp.obj.blur(temp.validation);
	temp.obj.bind("input propertychange", temp.validation);
	// 绑定[个人-邮箱]事件
	temp = io.personal.email;
	temp.obj.blur(temp.validation).blur(temp.ajax);
	temp.obj.bind("input propertychange", temp.validation);
	// 绑定[个人-手机验证码]事件
	...
	temp.obj.bind("input propertychange", temp.validation);
	// 绑定[个人-人机验证]事件
	var bindLuo = function() {
		setTimeout(function() {
			io.personal.luoValidation.obj = $(".sign-personal input[name=luotest_response]");
			if (io.personal.luoValidation.obj.length == 0) {
				bindLuo()
			} else {
				// 检测 螺丝帽 input 改变
				var val = io.personal.luoValidation.obj.val();
				setInterval(function() {
					if (val != io.personal.luoValidation.obj.val()) {
						val = io.personal.luoValidation.obj.val();
						io.personal.luoValidation.ajax();
					}
				}, 100);
			}
			;
